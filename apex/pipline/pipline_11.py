...
class piplineToolUI(QWidget):
    ...
    def saveCurrentENV(self):
        ...
        modMiddle_name = self.modMiddle_list.currentRow()
        modWipVer = self.modWip_version.currentRow()
        modPubVer = self.modPub_version.currentRow()
        polyVertexNorm = self.poly_vertex_normal.checkState()
        ...
        env_docs = {
                    ...
                    'shot_name': shot_name,
                    'modMiddle_name': modMiddle_name, 'modWipVer':modWipVer, 'modPubVer': modPubVer, 'polyVertexNorm': str(polyVertexNorm),
    ...
    def openCurrentENV(self):
        ...
        self.modMiddle_list.setCurrentRow(env_docs['modMiddle_name'])
        self.modWip_version.setCurrentRow(env_docs['modWipVer'])
        self.modPub_version.setCurrentRow(env_docs['modPubVer'])        
    ...
    def modelRefBtn(self):
        ...
        try:
            ...
            self.errorMessage(f'!! Select Model Pub !!')
            return 0
        ...
        print ('pub_path = '+pub_path)
        if not os.path.isfile(pub_path):
            self.errorMessage(f'!! alembic file does not Exist !!')
            return 0



        abc_import_cmd = 'AbcImport -mode import '
        abc_import_cmd += '"' + pub_path + '"'
        print ( 'abc_import_cmd : ' + abc_import_cmd)

        mel.eval(abc_import_cmd)
    ...
    def saveModPubBtn(self):
        ...
        pub_maya_path = self.getModSaveDir('output')+'/geometry/'+'/'+middle_name+'/'+wip_version+'/maya'
        
        pub_maya_file = f'{asset_name}_{middle_name}_{wip_version}.mb'
        print ('pub_maya_path = '+pub_maya_path )
        print ('pub_maya_file = '+pub_maya_file)
        ...
        pub_abc_path = self.getModSaveDir('output') + '/geometry/' + middle_name + '/' + wip_version + '/alembic'
        pub_abc_file = f'{asset_name}_{middle_name}_{wip_version}.abc'
        ...
        try:
            ...
            print(f'{asset_name}_grp does not exist!!')
        try:
            cmds.select('GEO', r=True)
        except:
    ...
    def assignShaderBtn(self):
        ...
        UpperNode = (cmds.listRelatives(cmds.ls(sl=True),parent=True))[0]
        selected_asset = self.asset_name.currentItem().text()
        selected = cmds.ls(sl=True)
        selectedLong = cmds.ls(sl=True, long = True)
        if not selected_asset.lower() in selectedLong[0].lower():
            self.errorMessage(f'!! This object is NOT {selected_asset} !!\n Select correct asset')
            return 0
        ...
        for sg in shader_dict['surface']:
            ...
            for obj in shader_dict['surface'][sg]['objects']:
                ...
                if '.f[' in obj:
                    ...
                    if not cmds.objExists(f"{UpperNode}|{obj.split('Shape')[0]}|{obj.replace(':', '')}"):
                        
                        self.errorMessage(f"!! Check the Shape Name of {obj.replace(':','')} !!")
                        cmds.select(UpperNode)
                ...
                if len(node) > 1:
                    for i in node:
                        if (selected_asset in i) and f'{UpperNode}|' in i:
                            cmds.select(i)
               
                if 'Shape' in obj: 
                    cmds.select(f"{UpperNode}|{obj.split('Shape')[0]}|{obj}")

                if '.f[' in obj: 
                    cmds.select(f"{UpperNode}|{obj.split('.f[')[0]}")
    ...
    def openModMidFileBrowser(self):
        
        department_dir = self.getDepartmentDir('model/work')
        modMid_dir = department_dir
        print('modMid_dir = '+ modMid_dir)
        os.startfile(modMid_dir)
    ...
    def initUI(self):
        ...
        self.poly_vertex_normal.toggle()
        ...
        self.modMiddle_list.itemDoubleClicked.connect(self.openModMidFileBrowser)
        ...
        modelA_layout.addWidget(self.poly_vertex_normal)
        ...
        self.wip_rig.setMinimumWidth(160)
        ...
        model_ref_btn = QPushButton('Model import')
        ...
        LookDevWip_version_layout.addWidget(model_ref_btn)
