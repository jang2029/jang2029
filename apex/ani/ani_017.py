...
class AniToolUI(QWidget):
    ...
    def getLayoutWip(self):
        ...
        for shot_name in sorted(os.listdir(layoutWip_path)):
            if os.path.isfile(f'{layoutWip_path}/{shot_name}'):
                if os.path.splitext(shot_name)[1] == '.mb':
                    self.layoutWip_list.addItem(shot_name)
    ...
    def getLayoutPub(self):
        ...
        for version in sorted(os.listdir(layoutPub_path)):
            for shot_name in os.listdir(f'{layoutPub_path}/{version}'):
                if os.path.isfile(f'{layoutPub_path}/{version}/{shot_name}'):
                    if os.path.splitext(shot_name)[1] == '.mb':
                        self.layoutPub_list.addItem(shot_name)
