...
4. slate.ma 는 슬레이트를 구성하는 마야 파일입니다. 레퍼런스로 불러와서 플레이블레스터를 만들고 자동으로 지워짐니다.
5. 마야씬안에 오디오노드는 최종 사용하는것 하나만 남겨 주세요. 여러게 있으면 원치않는 오디오로 영상이 만들어 짐니다.
6. 카메라의 디스플레이 옵션을 통일 했습니다.(resolutiom gate:on,safeaction:on,Horizental,DisplayOverscan=1,camera nearclip=10,panZoomEnabled= 0 )
7. 렌더옵션을 통일 했습니다. (아놀드렌더 셋업, 랜더사이즈 2560x1200, deviceAspectRatio, pixelAspect )
8. 알수없는 이유로 카메라의 Rotation Pivot 이 움직인 사례가 있어 모두 0,0,0 으로바꾸는  기능 추가 하였습니다.
'''
