...
class AniToolUI(QWidget):
    ...
    def aniSavePubBtn(self):
        ...
        if os.path.isfile(scene_csv):
            with open(scene_csv) as f:
                ...
                for asset_type, asset_name in data:
                    ...
                    abcPath = f'{shot_path}/{currentShot}/animation/output/animcache/animation_{asset_name}/{version}/alembic/'
                    if asset_type == 'character' or asset_type == 'prop':
                        
                        
                                
                        for i in ref_list:
                            if asset_name.lower() in i.lower():
                                if not os.path.isdir(abcPath):
                                    ...
                                    if len(bakeSet) == 0:
                                        self.errorMessage(f'{item} Error!!')
                                        continue
                                            
                                    

        if cmds.listRelatives('ETC|BAKE'):

            cmds.select('ETC|BAKE')
            selectedObj = cmds.ls(selection = True, l=True)
            bakeObj = ' -root '.join(selectedObj)
            abcPath = f'{shot_path}/{currentShot}/animation/output/animcache/animation_ETC/{version}/alembic/'
            if not os.path.isdir(abcPath):
                os.makedirs(abcPath)
        ...
        if cmds.listRelatives('BG'):
            cmds.select('BG')
            selectedObj = cmds.ls(selection = True, l=True)
            bakeObj = ' -root '.join(selectedObj)
            abcPath = f'{shot_path}/{currentShot}/animation/output/animcache/animation_BG/{version}/alembic/'
            if not os.path.isdir(abcPath):
                os.makedirs(abcPath)
        ...
        if cmds.listRelatives('BGPROP'):
            cmds.select('BGPROP')
            selectedObj = cmds.ls(selection = True, l=True)
            bakeObj = ' -root '.join(selectedObj)
            abcPath = f'{shot_path}/{currentShot}/animation/output/animcache/animation_BGPROP/{version}/alembic/'
            if not os.path.isdir(abcPath):
                os.makedirs(abcPath)
