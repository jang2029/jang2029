...
class ModelUI(QWidget):
    ...
    def getSaveDir(self, save_type=None):
        ...
        save_dir = f'{department_dir}/{middle_name}/{save_type}'
    ...
    def getVersion(self):
        ...
        self.publish_version.clear()
        pub_dir = self.getSaveDir('pub') + '/maya'
    ...
    def publishBtn(self):
        ...
        pattern_maya_str = f'/mod/{middle_name}/wip/maya'
        dst_maya_str = f'/mod/{middle_name}/pub/maya'
        pub_maya_path = re.sub(pattern_maya_str, dst_maya_str, self.wip_maya_path)
        # make publish maya dir
        pub_maya_dir = os.path.dirname(pub_maya_path)
        ...
        # publish alembic #
        ###################
        # make publish alembic path
        pattern_abc_str = f'/mod/{middle_name}/pub/maya'
        dst_abc_str = f'/mod/{middle_name}/pub/abc'
        pub_abc_path = re.sub(pattern_abc_str, dst_abc_str, pub_maya_path)
        pub_abc_path = re.sub('mb$', 'abc', pub_abc_path)
        # make publish alembic dir
        pub_abc_dir = os.path.dirname(pub_abc_path)
        if not os.path.isdir(pub_abc_dir):
            os.makedirs(pub_abc_dir)
        # select group
        cmds.select(f'{asset_name}_grp', r=True)
        # select group alembic export
        abc_export_cmd = 'AbcExport -j '
        abc_export_cmd += '"-frameRange 1 1 -uvWrite -dataFormat ogawa '
        abc_export_cmd += '-root |' + f'{asset_name}_grp '
